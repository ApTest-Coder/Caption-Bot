...
async def public_access_cb(query) -> bool:
    """Apply the same access rules to inline-menu clicks."""
    ...
    if await force_subscribed(query.bot, query.from_user.id):
        return True
    await query.answer(
        "🔒 Please join our channel first — use /start to get the join button.",
        show_alert=True,
    )
    return False
...
def _error_text(message: Message, error: Exception) -> str:
    """Build the owner-only error report."""
    return (
        "<b>🚨 Caption Bot Error</b>\n\n"
        f"<b>Channel:</b> {message.chat.title or message.chat.id}\n"
        f"<b>Message:</b> {message.message_id}\n"
        f"<blockquote expandable><b>Reason:</b> {str(error)[:3000]}</blockquote>"
    )
...
async def broadcast_command(message: Message) -> None:
    """Send an announcement to every tracked user."""
    if not await require_admin(message):
        return
...
async def set_public_command(message: Message) -> None:
    """Explain how deployment-level public mode is changed."""
    ...
    try:
        ...
        if kind == "caption":
            ...
            if filter_type not in VALID_FILTERS:
                await message.answer(
                    "❌ Valid: video/audio/document/photo/"
...
async def _send_caption(bot: Bot, message: Message, caption: str, markup):
    """Edit the current channel post caption."""
...
async def _copy_forward(bot: Bot, destination: int, message: Message):
    """Copy the current post to a configured destination."""
...
async def _retry_operation(operation, attempts: int = 2):
    """Retry the same Telegram operation when FloodWait is raised."""
...
async def channel_post(message: Message) -> None:
    """Process a connected channel post and apply its configuration."""
    ...
    try:
        ...
        if settings["forward"]["enabled"] and settings["forward"]["destination"]:
            await _retry_operation(
                lambda: _copy_forward(
                    message.bot,
                    settings["forward"]["destination"],
                    message,
                )
            )
    except Exception as exc:
